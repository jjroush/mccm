...
| Blue  | `inactive`   | At least one session is inactive |
| Red   | `needs_help` | At least one session needs help  |
| (off) | none         | No live sessions                 |

Blue stands in for the menu bar's *yellow* state — a yellow LED would match
the menu bar exactly, but red/blue/green is what's in the kit.
...
- **`CH343` port (next to the BOOT button)** — goes through a CH343P
  ...
  the chip when the port is opened. We don't use it.

GPIO pin choice — **GPIO18 (red), GPIO19 (blue), GPIO20 (green)**:
...
- C6 GPIOs swing 0 → 3.3 V. A red LED drops ~2.0 V, so a 330 Ω resistor
  passes (3.3 − 2.0) / 330 ≈ **4 mA**. Blue and green LEDs drop ~3.0 V,
  leaving only ~1 mA — visibly dimmer. That's fine for a status light;
  if you want them brighter, use 100 Ω for the blue and green legs.
